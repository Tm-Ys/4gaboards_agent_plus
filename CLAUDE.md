...
**当前位置**：任务二 P4 + 领域工具扩展完成（2026-06-16）。A 层 18 工具；三簇验证 view 3/4、list-view 4/6、card 10/21。
...
**之后**：① 补 card 剩余工具（manage_labels 标签选色、text_view md-editor 视图模式/resize/help）救剩余 card FAIL；② P5 变异测试；③ 前端可视化。

**已知问题（非工具缺陷）**：
- 弱网下批量 `page.goto` 登录/清理偶发 60s 超时（card 簇 4 异常皆此因）；cleanup 语言恢复在弱网下脆弱。
- 判官严格性：card-delete-via-bin（按 Delete 键 vs 点 bin 图标）、open-card-by-clicking-row（抓"点行空白区"操作方式）——核心目标其实达成。
- card 簇剩余 FAIL：manage_labels / view-activity-comment(波形图标) / text_view×3 待补工具。
...
- ⬜ **P5** 变异测试（提升档）+ 可视化前端（TS）。
- ⚠️ 已知：弱网下批量 `page.goto` 登录/清理偶发超时（非工具缺陷）；card 簇 manage_labels/text_view 等少数场景待补工具。
