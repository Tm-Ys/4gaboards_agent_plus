...
**当前位置**：任务二 P1.5 完成。基线 43%（30 个 easy+happy_path）→ settings 簇批量 68%（19 个）。
完整报告：`app/outputs/runs/settings-batch.json`（gitignored；可重跑复现）。

**P1.5 已完成（2026-06-15）**：preferences 用户级设置簇工具（`settings_open`/`settings_toggle`/`settings_select`）。
- 侦察证伪 handoff 原假设：instance 开关簇被 demo 站 `demoMode` 物理禁用（`<input disabled>`），不可救，已文档化为环境受限（`app/outputs/basic/README.md`）。
- 转向 preferences 簇：开关点 `span.switchRound`、下拉键入+Tab（复用 board_create）、按钮行（themeShape）双模；settings 簇 19 个批量 13 PASS / 6 FAIL（4 个 toggle FAIL 系 `settings-language` 切中文的批量副作用，英文 UI 下已验证 PASS；2 个表单 FAIL 非 P1.5 范围）。
- 已知副作用：批量串行跑 `settings-language` 切中文后污染后续 toggle 场景（英文开关名在中文 UI 失配）→ P4 批量 state 隔离/语言恢复。
...
- ✅ **P1.5 完成**：preferences 簇 settings_open/toggle/select 工具（settings 簇批量 68%；instance 开关簇因 demoMode 环境受限、已文档化）。下一步 P4 健壮性（批量 state 隔离 / 语言副作用恢复 / 拖拽）+ 表单类工具（account/edit-profile）+ 前端。
- ⬜ 可视化前端（TS）暂缓。
