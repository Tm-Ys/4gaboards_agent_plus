...
**当前位置**：任务二 P4 + 领域工具扩展 + **card 工具补全**（2026-06-16）。A 层 **20 工具**；card 簇补全后 manage_labels/text_view 五场景 **3/5 通过**（switch-modes / resize / manage-labels-2 PASS；open-help 环境受限 / manage-labels-1 复杂 medium）。
...
**之后**：① P5 变异测试（提升档）；② 前端可视化；③（可选）manage-labels-1 的 edit-rename 确认、open-help 的 popup 替代方案。
...
- ⚠️ 已知：弱网下批量 `page.goto` 登录/清理偶发超时（非工具缺陷）；open-help 受 @uiw window.open 限制；manage-labels-1 复杂 medium 待编排优化。
