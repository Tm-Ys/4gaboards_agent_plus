...
- **前置契约化（执行架构演进）**：当前前置依赖（preconditions）是自然语言、靠 ReAct 临场搭建（慢且不稳，前置失败会污染被测动作判定）。未来把前置做成工具静态元数据（`ToolDef.requires`：界面态 + 资源依赖），runner 执行前程序化补足（缺看板视图→`view_switch`、缺卡片→`card_create`），让「前置=确定性程序化调用 / 被测动作=ReAct 真实 UI」两分开；不回到生成端让 LLM 再产自然语言前置（会重蹈场景漂移）。

---

## 十、项目目标与评分标准对齐

> 本项目对照赛题评分标准的达成情况（基础功能档 + 提升创新档）。

### 基础功能档

| 赛题要求 | 达成 |
|---|---|
| 任务一：识别主要功能点，生成主要功能的测试场景，格式符合要求 | ✅ **118 功能点 / 179 场景**，格式 `[ [step]+ [expectation]? ]+` |
| 任务二：执行简单测试场景，验证执行完整性与功能正确性 | ✅ ReAct Agent + 独立判官，board 5/5、admin 3/3、account 1/1 全过 |

### 提升创新档

**任务一**
- **正确性与全面性**（信息真实有效、避免幻觉、全面覆盖）：✅ 长上下文直填（喂原文而非检索近似内容），**100% 功能覆盖**。
- **粒度与可执行性**（粒度合适、能被 Web Agent 有效执行）：✅ 合并同族微操作控粒度，179 场景均能被任务二实际跑起来。

**任务二**
- **通过率与稳定性**（简单/中等/困难场景正确通过、提升效率）：✅ 基线 **43%**、可测场景 **~80%**、settings 簇 **84%**。
- **变异测试**（识别执行异常 / 布局问题 / 语义错误）：✅ **P5 双层变异**——Layer1 改 expectation（spec-sensitivity）、Layer2 往真实轨迹注入故障（Mutation Score 33%）。
- **结果验证**（识别成功失败、准确输出失败原因）：✅ 独立判官 + **P6 双档严格性**量化「宽松代价」（lenient 46% ↔ strict 25%，trade-off 三角）。

> 额外创新（鼓励的创新方法与评估策略）：**docs→tools 桥**（任务一 feature catalog 自动脚手架任务二工具）、**复用轨迹模型**（1 次真跑 + N 次重判隔离判官）、**牛皮纸可视化控制台**（实时 ReAct 轨迹）—— 详见第八章。
