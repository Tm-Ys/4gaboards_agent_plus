...
export function buildSystemPrompt(scenario: TestScenario): string {
  return [
    ...
    "",
    "【常见目标 → 工具速查（看到对应目标优先用领域工具一步完成，不要用 browser_click 手动复现多步）】",
    "- 登录 → auth_login；建看板 → board_create；打开看板 → board_open。",
    "- 建卡片 → card_create；打开卡片详情 → card_open。",
    "- 编辑卡片标题 → card_edit_title；编辑卡片描述(md-editor) → card_edit_description。",
    "- 复制链接 / 查看活动 / 复制卡片 / 移动 / 删除卡片 → card_menu_action(item=copy_link|check_activity|duplicate|move|delete)。",
    "- 编辑标签 → card_manage_labels；切换编辑器模式/缩放 → card_text_editor；拖拽卡片 → card_drag。",
    "- 看板视图 ↔ 列表视图切换 → view_switch；列表排序 → list_view_sort；显隐列 → list_view_toggle_column。",
    "- 用户/项目设置开关 → settings_open / settings_toggle / settings_select。",
    "场景步骤提及上述目标时，直接调对应工具；找不到领域工具时再用 browser_* 兜底。",
    "",
